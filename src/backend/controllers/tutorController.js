...
exports.submitTutorForm = async (req, res) => {
  try {
    ...
    if (!syllabusFile || !cvFile) {
      return res
        .status(400)
        .json({ error: "Both syllabus and CV files are required." });
    }
    ...
    try {
      // Send verification email to the tutor
    ...
    // Save tutor to the database after emails are sent successfully
    ...
    res.status(201).json({
      message:
        "Tutor form submitted. Please check your email to verify your account.",
    });
  } catch (error) {
